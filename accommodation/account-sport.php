...
<li class="active"><a href="account-sport.php">Sports Bar</a></li>
</ul>
</div>
</div>
...
<tbody>
<?php
$query="select * from invoice where section_id=1 group by date_issued";
$numrows =count(getData($query));
...
$qry="select invoice_number, date_issued, sum(amount) as amount from invoice where section_id=1 group by date_issued limit $offset,$rowsperpage"; 
$invoices=getData($qry);
$total=0;
...
<td><?php echo $value['amount']; $total=$total+$value['amount'];?></td>
...
<?php endforeach; ?>
<tr class="table-content"><td colspan="2">Total Amount</td><td><?php echo number_format($total, 2, '.', ','); ?></td><td><img src="images/view.png" width="20"></td></tr>
</tbody>
</table>
<input type="submit" name="update" value="Update" /><input type="submit" name="delete" value="Delete" />
